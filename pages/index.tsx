  ...
  return (
    <div className={style.profile}>
      ...
      <main className=''>
        <figure>
          ...
          <figcaption className="max-w-7xl m-auto text-sm md:text-lg text-gray-600 pt-10 pl-2 pr-2 sm:pl-10 sm:pr-10 md:pl-30 md:pr-30 break-words">
            <h2 className="mb-5">Hi, my name is Jón Trausti and I'm from Iceland, Reykjavík.</h2>
            <p>
              I have passion for programming, solving problems and studying new tech.
              {` `}
              I've been programming since I was around 12 years old, which all started when I had the luck to tinker with <a target="_blank" rel="noreferrer" href="https://en.wikipedia.org/wiki/Delphi_(software)">Delphi</a>.
              {` `}
              I wrote my first online pictionary game called Skissa with ActionScript and .NET, which was popular in Iceland around 2007. I've written multiple websites, services and games throughout the years since then.
              {` `}
            </p>
            <p className="mt-5">
              More recently I've been writing services using .NET Core & Node.js and using managed solutions such as Lambda (FaaS) and API Gateway to run them in the cloud.
              {` `}
              For frontend I like to use React with Gatsby and Next.js, which are also hosted with <a target="_blank" rel="noreferrer" href="https://martinfowler.com/articles/serverless.html">serverless</a> solutions such as S3, CloudFront and Cloudflare Pages.
              {` `}
              I also have great experience with databases such as MSSQL and DynamoDB, but I always prefer managed solutions such as DynamoDB and S3, which has completely transformed how I think about databases today.
            </p>
            <p className="mt-5">
              I could talk about tech all day, so please don't hesitate to <a href="mailto:icewolfy@gmail.com" title="icewolfy@gmail.com">contact me</a> if you want to learn more!
            </p>
          </figcaption>
        </figure>
        <section className="mt-20 bg-gray-200 pt-10 pb-14 bg-gradient-to-br from-gray-200 to-gray-50">
          <h2 className="text-md text-center text-gray-400 uppercase italic font-bold">favorite tech topics</h2>
        ...
        <section className="mt-10 mb-20">
          ...
          <div className="text-lg align-text-top inline-block pt-6">
            {links.map(link => (
              <a target="_blank" key={link.url} href={link.url} className="flex px-5 py-2 items-center justify-center text-gray-600 rounded-full m-1 align-text-top opacity-75 hover:opacity-100 transition-opacity">
                <span className="flex">{link.text}</span>
                <svg xmlns="http://www.w3.org/2000/svg" className="flex ml-1 w-4 h-4 md:w-5 md:h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <footer className="text-center text-gray-400 mb-10">
        This page is written in Next.js and hosted in Cloudflare Pages.
      </footer>
    </div>
  )
}
